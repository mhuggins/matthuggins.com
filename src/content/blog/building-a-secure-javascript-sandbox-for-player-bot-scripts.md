...
In my upcoming collectible card game, [Codebound](https://codebound.io), players don't just pick cards and click buttons. They write TypeScript (or JavaScript) code that controls their deck. These scripts analyze the board state, make decisions, and execute actions autonomously. This creates a unique challenge: how do you let users run arbitrary code on your servers without compromising security?

You can't just `eval()` user code. That's a one-way ticket to a compromised server. But you also can't hobble the API so much that writing scripts becomes tedious. Players need to call functions, iterate over playable cards in their hand, and make decisions based on game state.

Let's take a look at how I approached building a sandboxed execution environment using V8 isolates while both maintaining security and   that's both secure and pleasant to use.
...
[**Resource exhaustion**](https://en.wikipedia.org/wiki/Resource_exhaustion_attack): A simple `while(true) {}` can peg your CPU. Allocating massive arrays can exhaust memory. Without limits, a single malicious script can take down your service.

[**Escape attacks**](https://en.wikipedia.org/wiki/Virtual_machine_escape): If user code can access Node.js globals like `require`, `process`, or `fs`, game over. They can read your environment variables, access your database, or worse.
...
And yet, despite all these restrictions, we still need to expose a rich API. Players should be able to write code like the following:
...
That means returning objects with methods -- not just raw data.

### Why Not Just Use vm or vm2?
...
For our API to be robust, though, it needs to expose methods on objects. When a player calls `card.play()`, something needs to happen in the game engine. How do we bridge this gap?

The naive approach would be to have players work with raw data, implementing every available function on the global object or an object that we make available to the global scope:
...
Instead, I wanted to implement something that is more object-oriented. Players should be able to interact with rich objects that have built-in functions. But how do we make that work across the isolate boundary?
...
The magic is in how we connect these two layers. We use `ivm.Callback` to register functions that can be called from inside the isolate:
...
But we don't just register callbacks and expose them directly. That would leak our implementation details and create security holes. Instead, we generate code that builds a proper API using these callbacks -- then hides them.

## Building APIs with Code Generation

### The API Builder DSL

We didn't want the sandbox to know anything about our game. The engine should define what the API looks like, and the sandbox should figure out how to expose it.

We created a declarative builder DSL for defining APIs:
...
### The API Interpreter: Where the Magic Happens
...
First, we take each callback from the API definition and register it with the isolate using `ivm.Callback`. Each callback gets a unique ID:
...
After this step, the `api` object inside the isolate has properties like `api._cb_0`, `api._cb_1`, etc. -- each one a bridge back to our Node.js handlers.

#### Step 2: Generate API Construction Code

Next, we generate JavaScript code that will run inside the isolate to build the actual API. For simple methods, this is straightforward:
...
Array methods with item builders are more interesting. We need to generate code that transforms each item in the array, attaching methods to each one:
...
We wrap everything in an [IIFE](https://developer.mozilla.org/en-US/docs/Glossary/IIFE), capture the callback references as local variables, delete them from the accessible `api` object, then freeze the API:
...
The key insight: we capture callbacks in local variables, then delete them from `api`. The closures still work (they reference the local variables), but user code can't access the raw callbacks.
...
The foundation is V8's isolate architecture. Each isolate is a separate heap with its own globals. There's no `require()`, no `process`, no `fs`. The only things that exist in the isolate are what we explicitly put there.
...
Even with process isolation, we harden the API layer:
...
Our test suite includes security-focused tests:
...
We test that internal properties are properly hidden, that enumeration attempts fail, and that there's no state leakage between executions.

## Interesting Patterns and Lessons Learned

### Pattern: Variant Item Builders
...
I solved this with variant item builders. The API definition includes multiple variants and a selector function that chooses which variant applies to each item:
...
### From Player Script to Executed Turn
...
**Complete decoupling**: The sandbox package knows nothing about cards, game rules, or our specific API. It just knows how to expose any API defined via `APIBuilder`. We could reuse this architecture for completely different games.
...
**Type safety**: Despite all the dynamic code generation, we maintain full TypeScript type safety. The `APIBuilder` is generic and produces typed definitions. Players get autocomplete in their editor.
...
**Debugging generated code**: When something goes wrong in generated code, stack traces point to code that doesn't exist in any source file. We added source annotations to make debugging easier.
...
Isolate creation has overhead -- roughly 10ms. But creating a context within an existing isolate is fast. We reuse isolates across turns, only creating fresh contexts for each execution.
...
Building a secure sandbox for untrusted code is hard, but `isolated-vm` provides a solid foundation. The key insights from our implementation:
...
**Defense in depth**: Process isolation, callback cleanup, deep freezing, fresh contexts -- each layer catches things the others might miss. Security isn't one feature; it's an architecture.

The result is a system where players can write expressive, natural JavaScript to control their bots, while we maintain complete control over what that code can actually do. They get a good developer experience. We get to sleep at night.
